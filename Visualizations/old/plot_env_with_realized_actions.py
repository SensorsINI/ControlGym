...
class EnvironmentPlotter(Plotter):
    def plot(
        ...
        realized_actions = np.expand_dims(realized_actions, 1) if realized_actions.ndim == 1 else realized_actions
        num_steps, num_actions = realized_actions.shape

        if self.fig is None:
            self.fig = plt.figure(figsize=(10, 5), layout="constrained", dpi=300.0)
            self.subfig_l, self.subfig_r = self.fig.subfigures(2, 1, hspace=0.1, height_ratios=[1, 3])
